...
class Options(object):
  ...
  def build_dataset(self,words, n_words):
    ...
    data = list() ## convert all the words into ids!!!!
    unk_count = 0
    for word in words:
    ...
    reversed_dictionary = dict(zip(dictionary.values(), dictionary.keys()))# id2words
    return data, count, reversed_dictionary # ids, frequencies, mapdic
  ...
  def generate_batch(self, window_size, batch_size, count):
    ...
    if data_index + span > len(data):  ## stop the training process???
      data_index = 0
      self.process = False  
